...
`bookkeeping.md`, `commit-policy.md`, `asking.md`.
...
3. **Propose the breakdown**:
   ...
   - **Tasks** per phase, each one reviewable in one sitting: title (imperative), `type` from
     ...
     predicted `files`, and **done criteria that are verifiable** — a command and its expected output,
...
4. **Show** the breakdown as a table (ID-to-be, title, type, phase, anchor, depends_on) plus the graph
...
5. **Write**, in dependency order:
   - each task with `rite new-task --cycle <cycle> --title ... --type ... --phase ... --depends-on ...
     --source-of-truth ... --json`, then fill its body (Goal, Scope, Done criteria, Notes) and its
