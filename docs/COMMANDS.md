...
| Subcommand | Does |
| --- | --- |
| `begin task\|fix\|review [--id ID] [--no-claim]` | resolve the cycle, take the item, return paths, config digest and commit template; `--no-claim` takes nothing (a run that only plans) |
