...
| `/rite:new-cycle <name> [--prefix X] [--plan P]` | `rite new-cycle`: folder, views, profile skeleton, pitfalls file; seeds the profile only with facts. |
...
| Subcommand | Does |
| --- | --- |
| `resolve-cycle [name]` | cycle chosen by the rules, with prefix, profile, pitfalls, plan |
